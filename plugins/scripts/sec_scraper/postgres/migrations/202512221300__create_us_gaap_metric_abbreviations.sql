...
CREATE TABLE IF NOT EXISTS sec_raw.us_gaap_metric_abbreviations (
    metric_name TEXT NOT NULL PRIMARY KEY,  -- Full US-GAAP metric name
    abbreviation TEXT NOT NULL UNIQUE,  -- Common abbreviation
    category TEXT,  -- Metric category
    description TEXT,  -- Brief description
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

COMMENT ON TABLE sec_raw.us_gaap_metric_abbreviations IS
'Mapping of US-GAAP metric names to common abbreviations for easier querying.';

-- Insert common metric abbreviations (use ON CONFLICT to handle re-runs)
...
('SellingGeneralAndAdministrativeExpense', 'SGA', 'Income Statement', 'Selling, general and administrative expenses'),
('ResearchAndDevelopmentExpense', 'RD', 'Income Statement', 'Research and development expense'),
...
('PropertyPlantAndEquipmentNet', 'PPE', 'Balance Sheet', 'Property, plant and equipment (net)'),
...
('PriceEarningsRatio', 'PE', 'Ratios', 'Price-to-earnings ratio'),
('ReturnOnAssets', 'ROA', 'Ratios', 'Return on assets'),
('ReturnOnEquity', 'ROE', 'Ratios', 'Return on equity'),
('CurrentRatio', 'CURR_RATIO', 'Ratios', 'Current ratio'),
('DebtToEquityRatio', 'DE', 'Ratios', 'Debt-to-equity ratio'),
...
('WorkingCapital', 'WC', 'Balance Sheet', 'Working capital')
